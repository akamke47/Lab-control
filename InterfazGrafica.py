            ...
            dcc.Tab(label='Controlador', children=[
                ...
                html.Div(id='Modos',className='row' ,children=[
                    html.Div(id='Manual', className='six columns', style={'color': colors['text'], 'borderStyle': 'solid', 'borderWidth': '5px', 'borderColor': '#B8860B'}, children=[
                        ...
                        html.Div(id='RazonesDiv', className='row', children=[
                            html.Div(id='Razon1Div', style={'paddingBottom':'50px'},className='six columns', children=[
                                ...
                                dcc.Slider(id='Razon1', min=0, max=1, step=0.05, value=0.6)]),
                    ...
                    html.Div(id='Automatico', className='six columns', style={'color': colors['text'], 'borderStyle': 'solid', 'borderWidth': '5px', 'borderColor': '#B8860B'}, children=[
                        ...
                        html.H4('Constantes del PID2'),
                        html.Div(id='constantes3', className='row', children=[
                            html.Div(id='P2', className='six columns', children=[
                                html.Label('Proporcional'),
                                dcc.Input(id='Kp2',placeholder='Ingrese un valor', type='text', value='0.5')]),
                            html.Div(id='I2', className='six columns', children=[
                                html.Label('Integral'),
                                dcc.Input(id='Ki2',placeholder='Ingrese un valor', type='text', value='0.3')])]),
                        html.Div(id='constantes4', className='row',  style={'paddingBottom':'10px'},children=[
                            html.Div(id='D2', className='six columns', children=[
                                html.Label('Derivativa'),
                                dcc.Input(id='Kd2',placeholder='Ingrese un valor', type='text', value='0.2')]),
                            html.Div(id='W2', className='six columns', children=[
                                html.Label('Anti wind-up'),
                                dcc.Input(id='Kw2',placeholder='Ingrese un valor', type='text', value='0')])])
                        ])
                    ]),
                html.Div(dcc.Graph(id='live-update-bar-graph')),
...
@app.callback(Output('live-update-bar-graph', 'figure'), [Input('intermediate', 'children')])
def update_bar_graph(alturas):
    global times, h1, h2, h3, h4
    alturas = json.loads(alturas)
    times.append(datetime.datetime.now())
    ...
    # Define el umbral para considerar un nivel bajo
    umbral_bajo = 10  # Define aquí el umbral deseado
    
    # Define los colores de las barras en función de las alturas
    colors = ['red' if h < umbral_bajo else 'blue' for h in [h1[-1], h2[-1], h3[-1], h4[-1]]]
    
    bars = go.Bar(
        x=['Tanque 1', 'Tanque 2', 'Tanque 3', 'Tanque 4'],
        y=[h1[-1], h2[-1], h3[-1], h4[-1]],
        name='Niveles de Tanque',
        marker=dict(color=colors)  # Aplica los colores definidos
    )
    
    layout = go.Layout(
        title='Niveles de Tanque en Tiempo Real',
        xaxis=dict(title='Tanques', color='white'),  # Texto de eje x en blanco
        yaxis=dict(title='Altura', color='white'),  # Texto de eje y en blanco
        paper_bgcolor='black',  # Fondo negro
        plot_bgcolor='black',  # Fondo de la gráfica negro
        font=dict(color='white'),  # Color del texto en blanco
        autosize=True
    )
    
    fig = {'data': [bars], 'layout': layout}
    return fig
...
@app.callback(Output('live-update-graph2', 'figure'),
              ...
              [State('Eleccion', 'value'), State('TipoManual', 'value'),
                ...
                State('Kp2', 'value'),
                State('Ki2', 'value'), State('Kd2', 'value'), State('Kw2','value'),
...
def SalidaControlador(alturas, eleccion, tipoManual, frec, amp, offset, fase, manualFijo,
                      Kp, Ki, Kd, Kw, Kp2, Ki2, Kd2, Kw2, SPT1, SPT2, guardando, formato, razon1, razon2):
